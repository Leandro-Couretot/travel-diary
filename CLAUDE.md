...
- **Deploy automático vía GitHub Actions (workflow ya escrito, falta cargar el secret)**: el usuario se encontró varias veces sin poder deployar desde el celular (Cloud Shell + login interactivo de Firebase es muy frágil en mobile — la sesión se corta al cambiar de app para copiar el link/código, ver conversación). Se evaluó n8n como alternativa y se descartó — no aporta nada que GitHub Actions no tenga ya integrado (clonado del repo, Node.js listo), y sumaría un servidor propio para mantener. El primer token que tenía el usuario a mano (uno de GitHub, para otro workflow suyo) **no sirve** — autentica contra GitHub, no contra Firebase/GCP, que es lo que pide `firebase deploy`.
  - **`.github/workflows/deploy.yml`**: dispara a mano (`workflow_dispatch`, botón "Run workflow" en la pestaña Actions — accesible desde la app/web de GitHub en el celular, eligiendo desde qué rama correr con el selector que ya trae GitHub) — corre `npm install` en `functions/` y `firebase deploy --non-interactive`, autenticado vía la variable de entorno `FIREBASE_TOKEN` (nunca hardcodeada en el archivo).
  - **Falta un solo paso, del usuario, desde la Mac** (no se puede hacer desde el celular, requiere login interactivo una vez): correr `firebase login:ci` (con `firebase login` ya hecho de antes) — imprime un token de texto en la terminal — y cargarlo en GitHub → Settings del repo → Secrets and variables → Actions → New repository secret, nombre `FIREBASE_TOKEN`, valor ese token. Nunca pasarlo por el chat con Claude.
  - Una vez cargado el secret, deployar desde cualquier lado (celular incluido) es: pestaña **Actions** del repo → "Deploy a Firebase (Hosting + Functions)" → **Run workflow**.
