...
- **Paso manual obligatorio, fuera del SQL:** Supabase (PostgREST) solo expone el schema `public` a su API por default. Hay que agregar `travel_diary` en Dashboard → Settings → Data API → "Exposed schemas" (queda `public, travel_diary`) — si no, toda query desde `functions/lib/supabase.js` falla en silencio del lado del servidor aunque el schema y las tablas existan bien. Costó detectar esto la primera vez (el síntoma fue: Drive se conectaba bien pero `establishSession()` nunca creaba la sesión de suscripción).
