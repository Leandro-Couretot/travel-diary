...
- **Manejo de errores de Drive (cuota excedida, archivos borrados a mano)**: plan completo pero **sin ejecutar** en `ERROR_HANDLING_PLAN.md` — a diferencia del de analytics, no depende de tener más usuarios, es una mejora de robustez que aplica desde el día 1. Cubre: cuota de Drive excedida al subir (hoy falla en silencio), archivos de media borrados manualmente desde Drive (hoy deja el ícono roto), y `day.json` borrado con la carpeta del día todavía con archivos (hoy el día entero desaparece de la app).
