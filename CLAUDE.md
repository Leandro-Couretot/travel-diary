...
- **Deploy automático vía GitHub Actions (pendiente de armar)**: el usuario se encontró varias veces sin poder deployar desde el celular (Cloud Shell + login interactivo de Firebase es muy frágil en mobile — la sesión se corta al cambiar de app para copiar el link/código, ver conversación). Se acordó como solución real: un workflow de GitHub Actions que corra `firebase deploy` (Hosting + Functions) solo, así deployar pasa a ser "aprobar/mergear desde la app de GitHub en el celular" en vez de terminal + login interactivo. Se evaluó n8n como alternativa y se descartó — no aporta nada que GitHub Actions no tenga ya integrado (clonado del repo, Node.js listo), y sumaría un servidor propio para mantener. **Falta hacer, desde la compu con `firebase login` a mano**: generar una clave de cuenta de servicio de Firebase/GCP (o un token con `firebase login:ci`) y cargarla como secret en el repo de GitHub, más escribir el workflow (`.github/workflows/deploy.yml`) que la use. El usuario ya tiene un token de GitHub de otro GitHub Actions suyo — probablemente no sirve tal cual para esto (lo que necesita `firebase deploy` es una credencial de Firebase/GCP, no un token genérico de GitHub), pero vale la pena chequear qué es exactamente antes de generar uno nuevo. Disparo del workflow: a definir si es automático en cada push a la rama de trabajo o manual (`workflow_dispatch`, un botón en la app de GitHub) — dado que hoy el deploy ya es un paso manual separado del push, probablemente `workflow_dispatch` sea lo más parecido al flujo actual.
