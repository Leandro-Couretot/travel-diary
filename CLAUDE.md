...
└── travel-diary/
    ...
    │   ├── book.json             ← orden manual del fotolibro (ver "Diario" → Tab Libro), opcional
...
- **Tab Libro:** vista de fotobook con layouts variados (1/2/3/4 fotos por página, max 4), swipe horizontal. Orden manual arrastrando fotos entre páginas — ver "Orden manual del fotolibro" (v1.23) y "Modo reordenar del fotolibro" (v1.24) para mover fotos entre páginas que no están juntas en pantalla
...
- **Modo reordenar del fotolibro (mover fotos entre páginas)** (v1.24): el usuario probó v1.23 en producción y reportó que arrastrar dentro de una misma página funcionaba, pero no podía cambiar una foto de página — causa real: cada página del Libro ocupa el 100% del ancho de pantalla (carrusel con swipe), así que nunca hay dos páginas visibles al mismo tiempo para soltar algo en la otra. Se agregó un botón "Reordenar" en `.book-controls` (`toggleBookReorderMode()`, solo visible si `albumCanEdit`) que cambia la vista del Libro a una grilla de páginas en miniatura (`renderBookReorderGrid()`/`buildBookReorderTile()`, tarjetas de ancho fijo ~150px en una tira con scroll horizontal — entran 2 en celular, más en pantallas anchas, sin codear breakpoints a mano) donde varias páginas sí están visibles a la vez. Reutiliza el mismo `initBookDragReorder`/`initBookTouchReorder`/`moveBookPhoto` de v1.23 sin cambios en la lógica de arrastre en sí (esas funciones ya operaban en abstracto sobre cualquier contenedor con celdas `.bp`) — `moveBookPhoto()` sí se actualizó para saber sobre qué contenedor está parado (`book-pages-container` o `book-reorder-container`, según `bookReorderMode`) al reconstruir la vista y restaurar el scroll tras cada drop. Para el caso de querer mover una foto a una página que ni siquiera es una de las visibles en la tira, se sumó `checkBookEdgeScroll()`/`clearBookEdgeScroll()`: sostener el arrastre cerca de un borde (~60px) medio segundo hace avanzar la tira sola a la siguiente tanda de páginas — mismo gesto que mover un ícono entre pantallas de inicio en iOS. Es un solo avance por acercamiento (no sigue empujando solo mientras el dedo/mouse esté quieto ahí sin moverse, porque en touch no llegan más eventos sin movimiento real) — hay que alejarse y volver a acercarse al borde para seguir avanzando más de una tanda; se evaluó y se dejó así por ahora al no justificarse la complejidad de un intervalo continuo para esta entrega. La misma detección de borde se sumó también a la vista inmersiva original (no solo a la grilla nueva), así que de paso también ahí se puede arrastrar hacia el borde para pasar de página sin soltar antes.

### Pendientes conocidos
- **Fotolibro: ajustar zoom/encuadre por foto y forzar layout por página**: pedido del usuario junto con el orden manual del fotolibro (v1.23), pero pospuesto — mucho más trabajo (gestos de arrastre/pellizco dentro de la foto para el encuadre, guardar un punto focal o layout elegido por página) y no era el problema urgente (que fotos de temas distintos quedaran mezcladas, resuelto con el reorder). Retomar si hace falta más control fino sobre el diseño de cada página.
